...
display_chapter_menu()
{
    echo -e "\n=================="
    echo "1: modules/hello"
    echo "2: modules/kobject"
    echo "3: modules/kobject-wq"
    echo "4: modules/timer"
    echo "5: modules/workqueue"
    echo "6: modules/wq-barrier"
    echo "7: drivers/gpio-interrupt"
    echo "8: drivers/gpio-io"
}

build_chapter()
{
    echo "arg num: $#, arg0: $0, arg1: $1, arg2: $2"
    export SOURCE_DIR=${TOP_DIR}/study-arm64-raspi
    export MODULES_DIR=modules
    export DRIVERS_DIR=drivers
    export HELLO_MOD_DIR=${SOURCE_DIR}/${MODULES_DIR}/hello
    export KOBJECT_MOD_DIR=${SOURCE_DIR}/${MODULES_DIR}/kobject
    export KOBJECT_WQ_MOD_DIR=${SOURCE_DIR}/${MODULES_DIR}/kobject-wq
    export TIMER_MOD_DIR=${SOURCE_DIR}/${MODULES_DIR}/timer
    export WQ_MOD_DIR=${SOURCE_DIR}/${MODULES_DIR}/workqueue
    export WQ_BARRIER_MOD_DIR=${SOURCE_DIR}/${MODULES_DIR}/wq-barrier

    if [ ! -d $TOP_DIR/output ]; then
        echo "build output directory is not exist. make new output dir..."
    ...
    case $selected in
        "1")
            echo "build module hello"
            make -C ${HELLO_MOD_DIR} clean
            make -C ${HELLO_MOD_DIR}
            cp -rf ${HELLO_MOD_DIR}/*.ko $TOP_DIR/output
            make -C ${HELLO_MOD_DIR} clean
            ;;
        *)
    ...
    if [ ! -d $TOP_DIR/mnt/root/home/pi/ldd ]; then
        sudo mkdir -p $TOP_DIR/mnt/root/home/pi/ldd
    fi

    sudo cp -vrf $TOP_DIR/output/* $TOP_DIR/mnt/root/home/pi/ldd
